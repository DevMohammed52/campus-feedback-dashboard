...
if mode == "Student View":
    ...
    with col1:
        ...
        st.markdown("Your voice matters. Share your thoughts anonymously and help improve campus life.")
            ...
            if submitted:
                if feedback_text.strip():
                    ...
                    if save_data_to_sheets(feedback_entry):
                        ...
                        st.success(f"Feedback submitted successfully! Detected Sentiment: **{sentiment}**")
    ...
    with col2:
        st.subheader("Why Submit Feedback?")
        st.markdown("""
        - **Anonymous & Safe**: Your identity is protected
        - **AI-Powered Analysis**: Advanced sentiment detection
        - **Real Impact**: Helps administration identify issues
        - **Quick & Easy**: Takes less than 2 minutes
        """)
        
        if len(st.session_state.feedback_data) > 0:
            st.markdown("---")
            st.metric("Total Feedback Submitted", len(st.session_state.feedback_data))
            st.caption("Your voice has been added to the community!")
